...
namespace
{
    inline float powerProcess(float buffer)
    {
        const float clampedBuffer = std::clamp(buffer, -1.0f, 1.0f);
        return clampedBuffer * std::abs(clampedBuffer);
    }
}
...
void PowerDistAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        ...
        {
            channelData[sample] = powerProcess(channelData[sample]);
        }
...
{
    return new PowerDistAudioProcessorEditor (*this);
}
